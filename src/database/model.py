...
class ProductType(sqla.types.TypeDecorator):
    impl = sqla.types.String

    def process_bind_param(self, value, dialect):
        if value is not None:
            return f"{value['Category']},{value['Variety']},{value['Packaging']},{value['Volume']},{value['Price_per_unit']}"
        return None
...
class Order(Base):  # type: ignore
    ...
    id = sqla.Column(sqla.Integer, primary_key=True, nullable=False)
    Type = sqla.Column(sqla.Enum(OrderTypeEnum), index=True, nullable=False)
    References = sqla.Column(sqla.Integer, sqla.ForeignKey("orders.id"))
    Products = sqla.Column(sqla.List(ProductType), nullable=False)
    Organisation_id = sqla.Column(sqla.Integer, sqla.ForeignKey("organisations.id"))
    Organization = sqla.relationship("Organisation", backref="orders")
...
class Organisation(Base):  # type: ignore
    ...
    Type = sqla.Column(sqla.Enum(OrganisationTypeEnum), index=True, nullable=True)
    Products = sqla.relationship("Order")
    Orders = sqla.relationship("Order")
